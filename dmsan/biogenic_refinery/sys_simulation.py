...
from dmsan.biogenic_refinery import scores_path
...
def get_uncertainties(country, N, seed=None, rule='L',
    ...
    if param_path:
        ...
        dfs = dict.fromkeys(('BiogenicRefinery A', 'BiogenicRefinery B'))
        for model in models:
            df = dfs[f'BiogenicRefinery {model.system.ID[-1]}'] = pd.DataFrame()
